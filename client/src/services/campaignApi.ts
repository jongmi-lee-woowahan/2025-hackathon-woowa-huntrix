    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        return {
          data: parsedData,
          success: true,
          message: `AI가 ${parsedData.length}개의 캠페인을 추천했습니다. (${attempt}번째 시도에서 성공)`
        }

      } catch (error) {
        console.error(`❌ Huntrix Agent-1 API 호출 실패 (시도 ${attempt}):`, error)
        lastError = error instanceof Error ? error : new Error('알 수 없는 오류가 발생했습니다.')
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          lastError = new Error('AI 분석이 5분을 초과하여 중단되었습니다.')
          break
        }
        
        // 네트워크 에러
        if (error instanceof TypeError && error.message.includes('fetch')) {
          lastError = new Error('네트워크 연결에 문제가 있습니다. 인터넷 연결을 확인해주세요.')
          break
        }
        
        console.log(`🔄 ${attempt + 1}번째 시도를 2초 후 진행합니다...`)
        await new Promise(resolve => setTimeout(resolve, 2000))
      }
    }
    
    console.error('🚨 Huntrix Agent-1 API 최종 실패:', lastError)
    throw lastError || new Error('AI 캠페인 추천 API 호출에 실패했습니다.')
  }
}
